...
export async function validateForm(event) {
    ...
    function ifContactFormValid() {
        if (checkLength(fullName.value,2) && checkLength(subjectId.value,4) && checkLength(messageId.value, 19) && validateEmail(email.value)) {
            // Add loader
            ...
            setTimeout(() => {
                ...
                submitButton.setAttribute("style", "color: #fff; background: #3a5964; border-color:#3a5964 ");
            ...
            // submitButton.innerHTML = "Submitted";
            // legend.innerText = "Summary";
            // formInput.forEach((item) => (item.style.border = "none"));
            // instruction.forEach((item) => (item.style.display = "none"));
            // messageInput.style.border = "none";
        }
    }
